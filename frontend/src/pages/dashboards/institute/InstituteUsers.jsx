...
function UserTable({ users, status, onApprove, onReject }) {
  if (users.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <EmptyState 
          title={`No ${status} Users Found`} 
          description={status === 'Pending' ? `There are no pending approval requests for users at the moment.` : `There are currently no active users in your organization.`}
          icon={
          ...
          <tbody className="divide-y divide-slate-100">
            {users.map(user => (
              <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 text-slate-600 text-sm capitalize">{user.role ? user.role.toLowerCase() : 'User'}</td>
...
function InstituteUsers() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [instructorsRes, learnersRes, pendingRes] = await Promise.all([
